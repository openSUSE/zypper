...
static MediaChangeReport::Action handle_common_options( unsigned reply, Url & url )
{
  ...
  switch ( reply )
  {
    ...
    case 3: /* change url */
    {
      ...
      Url newurl( get_text( _("New URI") + std::string(": "), url.asString() ) );
      url = newurl;
    }
    // fall through
    case 1: /* retry */
...
static MediaChangeReport::Action request_medium_https_handler( Zypper & zypper, Url & url )
{
  // https options
...
static void eject_drive_dialog( Zypper & zypper, Url & url, const std::vector<std::string> & devices, unsigned & index )
{
  bool cancel = false;
  if ( devices.empty() )
  {
...
namespace ZmartRecipients
{
								     ...
								     unsigned & index)
  {
    ...
    else if ( url.getScheme() == "cd" || url.getScheme() == "dvd" )
					     ...
					     media::AuthData & auth_data )
  {
    ...
    // curl authentication
    media::CurlAuthData * curl_auth_data = dynamic_cast<media::CurlAuthData*>(&auth_data);
    ...
    zypper.out().prompt( PROMPT_AUTH_PASSWORD, _("Password"), PromptOptions() );
