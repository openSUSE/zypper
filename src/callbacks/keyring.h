...
{ /////////////////////////////////////////////////////////////////
        ...
        if (_gopts.no_gpg_checks)
        {
          ...
          if (context.empty())
            ...
            Zypper::instance()->out().warning(boost::str(boost::format(
                ...
                % file % context.repoInfo().label() % id));
	...
	if (_gopts.gpg_auto_import_keys)
	  s << _("Automatically importing the following key:") << std::endl;
        ...
        if (!context.empty())
          s << str::form(_("Repository: %s"), context.repoInfo().label().c_str())
            << std::endl;
	...
	zypper.out().info( ColorString( COLOR_CONTEXT_MSG_WARNING, _(
