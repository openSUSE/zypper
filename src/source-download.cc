...
namespace
{
  ...
  void SourceDownloadImpl::buildManifest()
  {
    ...
    if ( ! pi.isDir() )
    {
      ERR << "Can't access download directory" << endl;
      throw( Out::Error( ZYPPER_EXIT_ERR_BUG,
			 boost::format(_("Can't access download directory '%s'.")) % pi.asString(),
	...
	return;
      ...
      for_( it, todolist.begin(), todolist.end() )
      {
  ...
  std::ostream & SourceDownloadImpl::dumpManifestTable( std::ostream & str )
  {
    ...
    for_( it, _manifest.begin(), _manifest.end() )
    {
      const Manifest::value_type & item( *it );
	...
	t << tr;
      ...
      {
	for_( it, spkg._packages.begin(), spkg._packages.end() )
	{
	  const PoolItem & pkg( *it );
	  TableRow tr;
