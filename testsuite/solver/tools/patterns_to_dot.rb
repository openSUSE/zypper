...
class Pattern
	...
	# save as dot to channel
	def save
		return if skip

		@channel.puts "\"#{name}\" {"
		@channel.print "  label = \"#{name}"
		unless provides.nil?
		    provides.each { |tag|
			@channel.print "|#{tag.name}"
		    }
		end
		@channel.puts "\""
		@channel.puts "};"
		...
		unless requires.nil?
		    requires.each { |dep|
			@channel.puts "\"#{name}\" -> \"#{dep.name}\" [ constraint = false ]" if dep.kind == "pattern"
		    }
		end

		unless conflicts.nil?
		    conflicts.each { |dep|
			@channel.puts "\"#{name}\" -> \"#{dep.name}\" [ dir = both ]" if dep.kind == "pattern"
		    }
		end
...
def usage
	STDERR.puts "Usage: patterns_to_dot.rb <patterns-file> [<patterns-file> ...]"
	exit 1
end
...
stream.puts "size = \"12,8\"; ratio = fill;"
ARGV.each do |arg|
